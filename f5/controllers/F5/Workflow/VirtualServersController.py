...
class F5WorkflowVirtualServersController(CustomController):
    ...
    def post(request: Request, assetId: int, partitionName: str) -> Response:
        ...
        try:
            if Permission.hasUserPermission(groups=user["groups"], action="workflow_virtualServers_post", assetId=assetId, partitionName=partitionName) or user["authDisabled"]:
