...
class CertificateUpdateWorkflow():
    def __init__(self, assetId: int, partitionName: str, profileType: str, profileName: str, user: dict, replicaUuid: str, *args, **kwargs):
    ...
    def __updateProfile(self, data: dict, virtualServerName: str = ""):
        ...
        if self.profileType != "client-ssl" and self.profileType != "server-ssl":
            raise CustomException(status=400, payload={"message": '"Update certificate: profile can be either "client-ssl" or "server-ssl".'})
        if virtualServerName:
            action += " of Virtualserver: " + virtualServerName

        try:
            Profile(self.assetId, self.partitionName, self.profileType, self.sslProfile).modify(data)
            self.__log(action="[WORKFLOW] " + action, objectType=self.profileType+" profile", object=str(data), status="updated")

        except Exception as e:
            self.__log(action="[ERROR] Workflow " + action, objectType=self.profileType+" profile", object=str(data), status="not updated")
            raise CustomException(status=400, payload={"message": f"Update {self.profileType} profile failed."})
