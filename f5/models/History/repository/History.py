...
class History:
    ...
    def list(username: str, allUsersHistory: bool) -> list:
        ...
        try:
            if allUsersHistory:
                c.execute("SELECT username, action, asset_id, config_object_type, config_object, status, date, dr_replica_flow FROM log ORDER BY date DESC")
            else:
                c.execute("SELECT username, action, asset_id, config_object_type, config_object, status, date, dr_replica_flow FROM log WHERE username = %s ORDER BY date DESC", [
                    username
                ])
